...
Updates Formula/cairn.rb in the Homebrew tap repo with release asset sha256
...
version="${tag#v}"
base_url="https://github.com/naoto256/cairn/releases/download/${tag}"
tap_repo="naoto256/homebrew-cairn"
branch="bump/cairn-${version}"
dry_run="${CAIRN_BREW_DRY_RUN:-0}"
...
formula="${tap_dir}/Formula/cairn.rb"
if [ ! -f "${formula}" ]; then
  echo "missing tap formula: ${formula}" >&2
  echo "create Formula/cairn.rb in ${tap_repo} from dist/brew/README.md first" >&2
  exit 1
fi
...
for target in "${assets[@]}"; do
  asset="cairn-v${version}-${target}.tar.gz"
  url="${base_url}/${asset}"
...
sha = ENV.fetch("SHA")
text = File.read(formula)
pattern = /(cairn-v#\{version\}-#{Regexp.escape(target)}\.tar\.gz"\n\s+)(?:# Fill after release\.\n\s+)?sha256 "[0-9a-f]{64}"/
...
if [ "${dry_run}" = "1" ]; then
  git diff -- Formula/cairn.rb
  exit 0
fi

git add Formula/cairn.rb
git commit -m "Update cairn ${tag} checksums"
git push -u origin "${branch}"
gh pr create \
  --repo "${tap_repo}" \
  --base main \
  --head "${branch}" \
  --title "Update cairn ${tag} checksums" \
