...
// Bumped to 4 for CodeRabbit follow-ups #2/#6: import-edge
// resolutions now set `target_qualified = None` to honor the
// `WorkspaceResolution` contract (path-scoped lookup no longer
// pins symbol_id for imports), and `resolve_dotted_to_qualified`
// adopts only namespace candidates that exist in the workspace
// (validate-before-adopt, kotlin-tier25 parity). Cached runs need
// invalidation.
...
pub fn analyze_files(
    ...
    // the persisted row lets `persist.rs` path-scoped lookup
