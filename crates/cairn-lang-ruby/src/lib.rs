...
fn doc_from_preceding_comments(node: Node<'_>, source: &[u8]) -> Option<String> {
    ...
    for sibling in parent.children(&mut cursor) {
        ...
        if sibling.kind() == "comment" {
            ...
            lines.push(strip_comment_marker(node_text(sibling, source)));
    ...
    // A blank line between the comment run and the declaration breaks
    // the association.
    if let Some(prev) = prev_end_row {
        if node.start_position().row > prev + 1 {
            return None;
        }
    }
