    ...
    ToolSpec {
        ...
        input_schema: json!({
            ...
            "properties": {
                ...
                "include_inherited": {"type": "boolean", "description": "When `container` is set, add declarations from transitive inheritance / implementation / mixin ancestors in the selected manifest. Filters and limit apply after the union. Syntactic-only or ambiguous ancestry returns usable partial results (`tier2_warming` / `inheritance_unresolved`) without guessing. Completeness carries one reason, so those semantic reasons take priority over `cap`; a `capped_increase_limit` hint still signals truncation. There is no cursor: retry with a larger limit or narrower filters."},
                ...
                "fuzzy":  {"type": "boolean", "description": "When `query` is set, run full-text matching over name + qualified + doc instead of exact name / qualified matching. Spaces between bare terms are AND, quoted text is an exact-order phrase, and prefix matching requires an explicit trailing `*`. Language punctuation such as `::` or `.` is treated as text."},
