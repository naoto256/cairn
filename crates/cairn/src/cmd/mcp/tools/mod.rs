...
pub(super) const VERBOSE_TIER3_DESC: &str = "Set `verbose_tier3=true` to include repo-wide analyzer readiness; default `tier3_status` only describes analyzers relevant to this query.";

pub(super) const COMPLETENESS_REASON_DESC: &str = "Results may carry `completeness: partial` with one of: `cap` — limit reached; raise `limit` to see more. `tier2_warming` — Tier-2 semantic analyzer is still indexing this snapshot. `tier3_warming` — Tier-3 workspace analyzer is still indexing. `tier3_unavailable` — Tier-3 binary was not found; Tier-1 / Tier-2 facts only. `analyzer_failed` — analyzer crashed on this snapshot; check `cairn ctl daemon status`. Set `verbose_tier3=true` to include repo-wide analyzer readiness; default `tier3_status` only describes analyzers relevant to this query.";
